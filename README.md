...
once (Settings → Pages) before the first deploy can succeed — this is a
one-time manual step GitHub does not expose through the API.

The site is served from `https://<owner>.github.io/bugatti-showcase/`, a
project page rather than the domain root, which is why `next.config.ts` sets
`basePath`/`assetPrefix` and every hardcoded asset path goes through
`lib/basePath.ts`'s `withBasePath()`.

This repository is private, but the published Pages URL is not — GitHub Pages
on a personal (non-Enterprise) account has no access control, so anyone with
the link can open the live site even though the source is hidden.
